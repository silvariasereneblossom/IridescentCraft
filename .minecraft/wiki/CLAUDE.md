...
- `iridescent_reforging-0.1.0.jar` — Tetra-armor extension. `ItemModularArmor extends ArmorItem implements IModularItem` (Java approach #1 — vanilla armor wiring stays inherited; portable Tetra logic reimplemented from `IModularItem` interface defaults). Four registered items (helmet/chestplate/leggings/boots) using vanilla iron as a placeholder ArmorMaterial. NBT-driven skin dispatch via `IClientItemExtensions.getHumanoidArmorModel` reads `tag.Skin`, looks up renderer in `SkinRegistry`, calls `prepForRender`. Skin definitions loaded from `data/<ns>/iridescent_reforging_skins/*.json` via `SkinDataLoader`; renderer factories registered per-source-mod (e.g. `IssRendererFactories`) gated by `ModList.isLoaded` at runtime. Custom `ConversionRecipe` consumes ISS armor + magic_cloth, outputs reforged variant with Apotheosis affix NBT and enchantments preserved. Workbench integration: phase 2 audit confirmed Tetra's `WorkbenchTile` accepts our items via the `IModularItem` interface alone (no integration code). Built from `iridescent-reforging-mod/` via `build_mod.sh`. Replaces Truly Modular: Armory; arsenal/archery retained.
