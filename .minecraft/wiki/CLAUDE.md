...
- `lovely_sparkle_pieces-0.1.0.0-iridescent.2.jar` — **Iridescent Lovely Pieces**: managed fork of [roylinglrl/NEOFORGE-LovelyPieces1.21.1](https://github.com/roylinglrl/NEOFORGE-LovelyPieces1.21.1) branch `1.20.1forge` @ `6dabd34` (upstream "Lovely Sparkle Pieces", non-commercial modpack use granted via README; the 1.20.1 Forge line is frozen upstream, we own the fork). ~60 Curios accessories + Gambler set-bonus system + tools. modid kept `lovely_sparkle_pieces` (asset/data namespace stability); display name + version rebranded. Built from `iridescent-lovely-pieces-mod/` via `./gradlew build`. Iridescent fixes: .1 = DAMAGE_MODIFIER attribute base 0->1 (upstream bug zeroed ALL player melee damage without an LSP curio), Blasphemous Contract auto-grant default OFF, AdventurersBelt permanent->transient modifiers, debug print removal. .2 (curation pass, docket #94) = fps_eye item DELETED (client-FPS-scaled damage), HeartSystem/heartrate HUD + damage-number particle system stripped (3 packets + particle type removed; PDA keeps time/speed), ALL crafting recipes removed - items enter ONLY via the pack's tier-scoped loot/boss pools (kubejs loot layer). The ~13 global Curios slot rows are KEPT by operator decision (auto-affixable under Apothic Curios). Damage layering: multiplies on LivingDamageEvent LOWEST = after our DamageModifierRegistry (LivingHurtEvent NORMAL) by pipeline order.
