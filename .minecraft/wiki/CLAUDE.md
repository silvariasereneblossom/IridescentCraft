...
- `iridescent_biomes-1.0.0.jar` — Custom TerraBlender region registration for icraft:cherry_river_meadow + icraft:cherry_mountains. TerraBlender 3.x on 1.20.1 needs Java-side region registration; datapacks alone can't assign parameter points. Built from `iridescent-biomes-mod/` via `./gradlew build` (ForgeGradle toolchain). Biome JSONs + `is_overworld` tag + en_us lang entries ship inside the jar. **If you delete this jar, the custom biomes will stop spawning** (but stay registered — Nature's Compass will show them with empty Dimension). Bumping biome content or parameter points requires a rebuild.
