...
Current custom JARs:
- `iridescent_codex_data.jar` — Patchouli Codex book, shipped as a Forge content mod (`lowcodefml` loader, no Java). Ships `META-INF/mods.toml` so Forge loads it at mod-load time. **Belt-and-suspenders:** identical `book.json` is ALSO present at `kubejs/data/icraft/patchouli_books/iridescent_codex/book.json` (with full content in `kubejs/assets/...`) as a fallback — whichever registration path Patchouli honors, the book ends up registered consistently. Built from `datapack_sources/iridescent_codex/` via `build_codex.sh`. Do NOT re-add the Paxi zip variant — that had mismatched `use_resource_pack` and was the original source of "Invalid book ID" conflicts.
