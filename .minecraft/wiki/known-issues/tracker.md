...
- **Confirmed:** 10x `/loot give @s loot apotheosis:chests/tome_tower` produced 1 book against an expected ~34 -- 97% suppression. Artifacts at 0/10 was inconclusive on its own (43% probability under normal luck) but combined with the books strongly suggested the filter was the suspect.
- **Root cause:** `lootjs_overhaul.js:115` registered a chest-wide `addLootTypeModifier(LootType.CHEST).removeLoot(ItemFilter.custom(blankEnchantedBookFilter))` predicate (added 2026-04-19, rewritten 2026-04-20 / -21 / -22). The predicate's regex + substring patterns expected Forge 1.20.1's NBT toString output to use no-whitespace format `StoredEnchantments:[{id:"X:Y"...}]`. Actual NBT has whitespace after colons: `StoredEnchantments: [{id: "X:Y", lvl: Ns}]`. Predicate never matched -> filter treated every enchanted book as blank -> stripped them all.
- **Resolved:** Removed lines 91-115 from `lootjs_overhaul.js`. Added `kubejs/server_scripts/diag_blank_book_trace.js` to log any future blank enchanted_book entering a player inventory with full context (player, dimension, position, NBT). If real upstream blank-book sources still exist, the tracer will localize them for a targeted per-mod fix instead of a chest-wide filter. Synced to all 3 distros.
- **Related (resolved 2026-04-19):** Earlier same-day fix switched LootJS additions from `LootEntry.of('minecraft:enchanted_book')` to `LootEntry.of('minecraft:book').enchantWithLevels(...)` -- that fix is preserved and was the correct solution. The chest-wide filter was an over-aggressive belt-and-suspenders measure that turned out to be the suspenders strangling the wearer.
